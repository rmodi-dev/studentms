...
exports.TotalFeesInPeriod = async(req, res) => {
    ...
    .then(data => {
        function newPayment(fees_schedule){
            const feespayments = [];
            let total_paid = 0;
        ...
        data.forEach(student => {
            ...
            if(student.studentfees.length > 0){
                student.studentfees.forEach(feesSchedule => {
                    const {total, payments} = newPayment(feesSchedule.feespayments);
                    const new_fees_schedule = {
                        "class": student.class,
                        "fees_amount": feesSchedule.fees_amount,
                        "outstanding_balance": feesSchedule.fees_amount - total,
                        "total_paid": total,
